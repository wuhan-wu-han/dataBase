...
功能：将管廊环境传感器时序数据发送到Kafka消息队列
支持JSON格式消息序列化，全量数据与告警数据分流到不同Topic

Topic：
    tunnel-sensor-topic   全量管廊环境监测数据
    tunnel-alarm-topic    告警旁路（alarm_code != 0，key 为点位编码）

使用示例：
    python tunnel_kafka_producer.py
    python tunnel_kafka_producer.py --input data/logs/tunnel_sensor_20260828.log
...
# Kafka Topic配置
SENSOR_TOPIC = "tunnel-sensor-topic"
ALARM_TOPIC = "tunnel-alarm-topic"
...
def send_realtime_data(producer, duration=60):
    ...
    Args:
        ...
        duration (int): 运行时长（秒）
    """
    from generate_tunnel_logs import SENSOR_POINTS, generate_log_entry

    point_count = len(SENSOR_POINTS)
    print(f"实时模式：{point_count}个点位，运行{duration}秒")
    ...
    while time.time() - start_time < duration:
        point = SENSOR_POINTS[idx % point_count]
        record = generate_log_entry(point, datetime.now(), idx // point_count)
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="Kafka管廊环境数据生产者")
