  ...
  { path: '/403', name: 'Forbidden', component: () => import('@/views/Forbidden.vue'), meta: { title: '无权访问' } },
  {
    ...
    children: [
      // ===== 主平台已实现模块 =====
      {
        ...
        meta: { title: '监控大屏', permission: 'dashboard:view' }
      },
      {
        path: 'gis',
        name: 'GISSituation',
        ...
        meta: { title: '综合态势', fullBleed: true, permission: 'gis:view' }
      },
      {
        path: 'alerts',
        name: 'AlertList',
        component: () => import('@/views/AlertList.vue'),
        meta: { title: '预警事件', permission: 'alert:view' }
      },
      {
        path: 'alerts/:id',
        name: 'AlertDetail',
        component: () => import('@/views/AlertDetail.vue'),
        meta: { title: '预警详情', permission: 'alert:view' }
      },
      {
        path: 'rules',
        name: 'RuleManage',
        component: () => import('@/views/RuleManage.vue'),
        meta: { title: '规则管理', permission: 'alert:manage' }
      },
      {
        path: 'failure-prediction',
        name: 'FailurePrediction',
        component: () => import('@/views/FailurePrediction.vue'),
        meta: { title: '故障预报', permission: 'failure:view' }
      },
      ...
      // ===== 已迁移模块：路由直接跳转到本地页面组件 =====
      {
        ...
        meta: { title: '燃气风控', permission: 'gas-risk:view' }
      },
      {
      ...
      },
      {
        ...
        meta: { title: '道路塌陷', permission: 'road-hazard:view' }
      },
      ...
      // ===== 新增业务模块 =====
      {
        ...
        meta: { title: '风险研判', permission: 'risk:view' }
      },
      {
        path: 'hazmat',
        name: 'Hazmat',
        component: () => import('@/views/hazmat/Index.vue'),
        meta: { title: '危化品监管', permission: 'hazmat:view' }
      },
      {
        path: 'utility-tunnel',
        name: 'UtilityTunnel',
        component: () => import('@/views/tunnel/Index.vue'),
        meta: { title: '综合管廊', permission: 'tunnel:view' }
      },
      {
        path: 'emergency-plan',
        name: 'EmergencyPlan',
        component: () => import('@/views/emergencyPlan/Index.vue'),
        meta: { title: '应急预案', permission: 'plan:view' }
      },
      {
        path: 'asset-cost',
        name: 'AssetCost',
        component: () => import('@/views/assetCost/Index.vue'),
        meta: { title: '资产成本', permission: 'asset-cost:view' }
      },
      {
        path: 'work-order',
        name: 'WorkOrder',
        component: () => import('@/views/workOrder/Index.vue'),
        meta: { title: '工单管理', permission: 'work-order:view' }
      },
      {
        // 市政井盖全生命周期管控
        path: 'manhole-cover',
        ...
        meta: { title: '井盖管控', permission: 'tunnel:view' }
      },
      {
        // 供水管网精细化管控
        path: 'water-supply',
        ...
        meta: { title: '供水管控', permission: 'tunnel:view' }
      },
      {
        path: 'users',
        name: 'UserManagement',
