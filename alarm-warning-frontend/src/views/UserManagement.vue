  ...
  <section class="users-page">
    ...
    <el-card shadow="never">
      <el-table v-loading="loading" :data="users">
        ...
        <el-table-column prop="emailMasked" label="邮箱" min-width="180"><template #default="{ row }">{{ row.emailMasked || '未绑定' }}</template></el-table-column>
        <el-table-column prop="phoneMasked" label="手机号" min-width="140"><template #default="{ row }">{{ row.phoneMasked || '未绑定' }}</template></el-table-column>
    ...
    <el-dialog v-model="editorVisible" :title="editing ? '编辑用户' : '新增用户'" width="460px">
      <el-form label-position="top">
        ...
        <el-form-item label="邮箱"><el-input v-model.trim="form.email" placeholder="用于接收告警邮件" /></el-form-item>
        <el-form-item label="手机号"><el-input v-model.trim="form.phone" maxlength="11" placeholder="用于接收短信通知" /></el-form-item>
        <el-form-item label="部门/区域"><el-input v-model.trim="form.departmentId" placeholder="可选，用于区域通知" /></el-form-item>
...
const form=reactive({ username:'', displayName:'', password:'', roles:['viewer'], enabled:true, email:'', phone:'', departmentId:'' })
...
function openCreate(){ editing.value=false; Object.assign(form,{username:'',displayName:'',password:'',roles:['viewer'],enabled:true,email:'',phone:'',departmentId:''}); editorVisible.value=true }
function openEdit(row){ editing.value=true; selected.value=row; Object.assign(form,{username:row.username,displayName:row.displayName,password:'',roles:[...row.roles],enabled:row.enabled,email:row.email||'',phone:row.phone||'',departmentId:row.departmentId||''}); editorVisible.value=true }
async function save(){ if(!form.username||!form.displayName||!form.roles.length||(!editing.value&&!form.password)) return ElMessage.warning('请完整填写用户信息'); saving.value=true; try{ const contact={email:form.email,phone:form.phone,departmentId:form.departmentId}; if(editing.value) await updateUser(selected.value.id,{displayName:form.displayName,roles:form.roles,enabled:form.enabled,...contact}); else await createUser({username:form.username,displayName:form.displayName,password:form.password,roles:form.roles,...contact}); ElMessage.success('保存成功'); editorVisible.value=false; await load() }catch(e){ ElMessage.error(e.response?.data?.detail||'保存失败') }finally{ saving.value=false } }
