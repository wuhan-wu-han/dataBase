...
public class GasRiskEventConsumer {

    private final ObjectMapper objectMapper;
    private final AlertEngineService alertEngineService;

    @KafkaListener(
            topics = "${alert.kafka.topics.gas-risk-event}",
    ...
    public void consume(String message) {
        try {
            ...
            if (kafkaMessage.getEventId() == null || kafkaMessage.getSource() == null
                ...
                log.warn("Invalid gas risk message, missing required fields: {}", message);
                return;
            }

            log.info("Received gas risk event: eventId={}, deviceId={}, eventType={}",
                    kafkaMessage.getEventId(), kafkaMessage.getDeviceId(), kafkaMessage.getEventType());

            alertEngineService.processMessage(kafkaMessage);
        } catch (Exception e) {
            log.error("Kafka message consume failed, topic=gas-risk-event-topic, message={}", message, e);
        }
    }
}
