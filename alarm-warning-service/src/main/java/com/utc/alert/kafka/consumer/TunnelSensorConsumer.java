...
public class TunnelSensorConsumer {

    private final ObjectMapper objectMapper;
    private final AlertEngineService alertEngineService;

    @KafkaListener(
            topics = "${alert.kafka.topics.tunnel-sensor}",
    ...
    public void consume(String message) {
        try {
            ...
            if (kafkaMessage.getEventId() == null || kafkaMessage.getSource() == null
                ...
                log.warn("Invalid sensor message, missing required fields: {}", message);
                return;
            }

            log.info("Received sensor data: eventId={}, deviceId={}, eventType={}",
                    kafkaMessage.getEventId(), kafkaMessage.getDeviceId(), kafkaMessage.getEventType());

            alertEngineService.processMessage(kafkaMessage);
        } catch (Exception e) {
            log.error("Kafka message consume failed, topic=tunnel-sensor-topic, message={}", message, e);
        }
    }
}
